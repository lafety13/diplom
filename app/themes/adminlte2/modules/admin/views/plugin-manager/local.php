...
$this->params['breadcrumbs'][] = 'Plugin management';
?>
<style>
...
<?php
$items = [
    ['label'=>'All','url'=>['local','tab'=>'all'],'active'=>'true'],
    ['label'=>'Installed','url'=>['local','tab'=>'setuped'],'active'=>'true'],
    ['label'=>'New','url'=>['local','tab'=>'new'],'active'=>'true'],
];
...
if(is_array($result) && isset($result['data'])){
	foreach($result['data'] as $v){
		if(is_array($v)){
			$author = isset($v['config']['author']) ? "Author:".$v['config']['author'] :'';
			$email = isset($v['config']['email']) ? "Email:".$v['config']['email'] :'';
			...
			if(!empty($dependencies)){
				$v['config']['description'] .= "<br/>"."Dependencies:".$dependencies;
			}
            ...
            $btn_unsetup_label = Html::tag('i','Unloading',['class'=>'fa fa-edit']);
			$btn_unsetup = Html::a($btn_unsetup_label,'#',['class' => 'unsetup btn btn-xs btn-success','style'=>'','data-toggle' => 'modal']);

			$btn_delete_label = Html::tag('i','Delete',['class'=>'fa fa-trash']);
...
echo GridView::widget([
    ...
    'columns' => array(
        ...
        array('attribute'=>'name', 'header'=>'Name','options'=>array('style'=>'width:15%')),
        array('attribute'=>'type', 'header'=>'Type','options'=>array('style'=>'width:5%')),
        array('attribute'=>'version', 'header'=>'Version','options'=>array('style'=>'width:5%')),
        array('attribute'=>'description', 'header'=>'Description','format' => 'raw','options'=>array('style'=>'width:45%')),
        array('attribute'=>'_action_','header'=>'Action','format' => 'raw','options'=>array('style'=>'width:15%')),
    ),
]);
?>
</div>
...
{
	if('delete'==action){
        yii.confirm("Are you sure you want to delete it？",function () {
            doAction(o,action);
        },function () {
            return;
        });
	}else if('unsetup'==action){
        yii.confirm("Are you sure you want to uninstall？",function () {
            doAction(o,action);
        },function () {
            return;
        });
...
Modal::begin([
'id' => 'install-modal',
'header' => '<div class="modal-title">Install the plugin: <b>Menu management</b></div>',
